...
class FashionMNISTClassificationTask(SequentialTask):
    """
    Create a new task based around classifying between Fashion MNIST images 
    ...
    def __init__(self, 
        ...
        """
        Create a new Fashion MNIST classification task.
        ...
        self.training_batches = training_batches \
            ...
            else int(FashionMNISTClassificationTask.ds_info.splits["train"].num_examples/batch_size)
        self.validation_batches = validation_batches \
            if validation_batches!=0 \
            else int(FashionMNISTClassificationTask.ds_info.splits["test"].num_examples/batch_size)
    ...
    def create_datasets(self) -> Tuple[tf.data.Dataset, tf.data.Dataset]:
        ...
        training_dataset = FashionMNISTClassificationTask.full_training_dataset \
        ...
        validation_dataset = FashionMNISTClassificationTask.full_validation_dataset \
            .filter(lambda _, label: tf.reduce_any(tf.equal(label, filter_range)))
